...
require_once dirname(__DIR__) . "/app/presenter/inventarioPresenter.php";
$presenter = new InventarioPresenter();
                ...
                <?php while ($fila = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td class="text-center">
                            <button class="btn btn-info btn-sm btnEditar" data-id="<?= $fila['id_producto'] ?>"
                                data-cod="<?= htmlspecialchars($fila['codigo']) ?>"
                                data-nom="<?= htmlspecialchars($fila['nombre']) ?>"
                                data-des="<?= htmlspecialchars($fila['descripcion']) ?>" data-can="<?= $fila['cantidad'] ?>"
                                data-pre="<?= $fila['precio_unidad'] ?>" data-toggle="modal"
                                data-target="#modalEditar">Editar</button>
                            <button class="btn btn-danger btn-sm"
                                onclick="confirmarEliminar(<?= $fila['id_producto'] ?>, '<?= $fila['nombre'] ?>')">Borrar</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
            ...
            <div class="modal-content border-0">
                <form method="POST">
                    ...
                    <div class="modal-footer"><button type="submit" name="registrar"
                            class="btn btn-success btn-block">Guardar en Base de Datos</button></div>
                </form>
            </div>
        </div>
    </div>
                ...
                <form method="POST">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                <label>Precio Unitario</label>
                                <input type="number" step="0.01" name="precio_unidad" id="edit_pre" class="form-control"
                                    required>
                            </div>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="modal-footer"><button type="submit" name="editar"
                            class="btn btn-info btn-block">Actualizar Datos</button></div>
                </form>
            </div>
        </div>
    </div>
