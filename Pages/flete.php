...
require_once dirname(__DIR__) . "/app/presenter/fletePresenter.php";
$presenter = new FletePresenter();
$data = $presenter->manejarPeticiones();

$result = $data['result'];
$filtro_actual = $data['filtro_actual'];
?>
        ...
        <div class="row mb-4 align-items-center">
            <div class="col-md-7">
                ...
                <div class="btn-group shadow-sm bg-white p-1 rounded mt-2">
                    <a href="?filtro=dia"
                        class="btn btn-<?= $filtro_actual == 'dia' ? 'primary' : 'light' ?> btn-sm">Hoy</a>
                    <a href="?filtro=semana"
                        class="btn btn-<?= $filtro_actual == 'semana' ? 'primary' : 'light' ?> btn-sm">Semana</a>
                    <a href="?filtro=mes"
                        class="btn btn-<?= $filtro_actual == 'mes' ? 'primary' : 'light' ?> btn-sm">Mes</a>
                    <a href="?filtro=todo"
                        class="btn btn-<?= $filtro_actual == 'todo' ? 'primary' : 'light' ?> btn-sm">Todo</a>
                </div>
            </div>
            <div class="col-md-5 text-md-right">
                <button class="btn btn-success px-4 font-weight-bold shadow btn-new" data-toggle="modal"
                    data-target="#modalFlete">+ Nuevo Flete</button>
            </div>
        </div>
                ...
                <table id="tablaFletes" class="table table-hover w-100 dt-responsive nowrap">
                    ...
                    <tbody>
                        <?php while ($fila = $result->fetch_assoc()): ?>
                            <tr>
                                <td class="align-middle font-weight-bold"><?= date("d/m/Y", strtotime($fila['fecha'])) ?>
                                </td>
                                ...
                                <td class="align-middle font-weight-bold text-dark">$<?= number_format($fila['valor'], 2) ?>
                                </td>
                                <td class="text-center align-middle">
                                    <button
                                        onclick="confirmarCambio(<?= $fila['id'] ?>, <?= $fila['cancelado'] ?>, '<?= $filtro_actual ?>')"
                                ...
                                <td class="text-center align-middle">
                                    <div class="btn-group shadow-sm">
                                        <button class="btn btn-outline-info btn-sm btnEditar" data-id="<?= $fila['id'] ?>"
                                        ...
                                        <button onclick="confirmarEliminar(<?= $fila['id'] ?>, '<?= $filtro_actual ?>')"
            ...
            <div class="modal-content border-0 shadow-lg">
                <form method="POST">
                    ...
                    <div class="modal-body p-4">
                        <div class="form-group"><label class="small font-weight-bold">FECHA</label><input type="date"
                                name="fecha" class="form-control" required value="<?= date('Y-m-d') ?>"></div>
                        <div class="form-group"><label class="small font-weight-bold">ORIGEN</label><input type="text"
                                name="origen" class="form-control" placeholder="Ciudad de salida" required></div>
                        <div class="form-group"><label class="small font-weight-bold">DESTINO</label><input type="text"
                                name="destino" class="form-control" placeholder="Ciudad de llegada" required></div>
                        <div class="form-group"><label class="small font-weight-bold">MONTO PACTADO ($)</label><input
                                type="number" step="0.01" name="valor"
                                class="form-control form-control-lg text-primary font-weight-bold" placeholder="0.00"
                                required></div>
                    </div>
            ...
            <div class="modal-content border-0 shadow-lg">
                <form method="POST">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="form-group"><label class="small font-weight-bold">FECHA</label><input type="date"
                                name="fecha" id="edit_fecha" class="form-control" required></div>
                        <div class="form-group"><label class="small font-weight-bold">ORIGEN</label><input type="text"
                                name="origen" id="edit_origen" class="form-control" required></div>
                        <div class="form-group"><label class="small font-weight-bold">DESTINO</label><input type="text"
                                name="destino" id="edit_destino" class="form-control" required></div>
                        <div class="form-group"><label class="small font-weight-bold">MONTO ($)</label><input
                                type="number" step="0.01" name="valor" id="edit_valor"
                                class="form-control form-control-lg font-weight-bold" required></div>
                    </div>
                    <div class="modal-footer bg-light">
